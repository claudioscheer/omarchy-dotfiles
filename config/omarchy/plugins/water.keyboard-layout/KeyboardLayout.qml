...
BarWidget {
  ...
  readonly property string layoutLabel: resolvedLabel(layoutFull, layoutBriefs, root.setting("labels", null))

  // Optional shell.json labels map, keyed by the xkb tooltip, e.g.
  // "English (US, intl., with dead keys)": "PT"
  function resolvedLabel(description, briefs, labels) {
    if (description && labels) {
      if (typeof labels[description] === "string" && labels[description])
        return labels[description].substring(0, 3).toUpperCase()
      for (var key in labels) {
        if (String(key) === String(description) && typeof labels[key] === "string" && labels[key])
          return labels[key].substring(0, 3).toUpperCase()
      }
    }
    return KeyboardLayoutModel.shortLabel(description, briefs)
  }
